...
<template>
	<div class="bgcolor">
		...
		<cdxxb v-for="(item,index) in num" :key="item.id" :getSavedData="item" :value="djyz" @childData="getBack"></cdxxb>
		<!-- <p class="add">
			<span @click="add">添加</span>
		</p> -->
		<div class="nextpage">
			<mt-button size="large" type="primary" @click="complete">完成</mt-button>
		</div>
			...
			getBack(val){
				...
				if(this.container.length==this.num.length){
					...
					if([...new Set(this.container)][0]==true&&[...new Set(this.container)].length==1){//返回true跳转，否则继续提示完善信息
						...
						console.log(value)
						console.log("qqqqq");
						sendWsxxData({"param":encodeURI(value)}).then((res) => {
							if(res.code === '40001'){
								//调用接口传递已完成状态
